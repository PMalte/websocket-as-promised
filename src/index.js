...
class WebSocketAsPromised {
   ...
   * a pending request with the given requestId exists and isPossibleResponse
   * returns true (which does by default).
   *
   * @see https://vitalets.github.io/chnl/#channel
   * @example
  ...
  /**
   * Event channel triggered every time when request comes.
   * Received message considered a request if it is not considered a response.
   *
   * @see https://vitalets.github.io/chnl/#channel
   * @example
   * wsp.onRequest.addListener(data => console.log(data));
   *
   * @returns {Channel}
   */
   get onRequest() {
  ...
  removeAllListeners() {
    ...
    this._onRequest.removeAllListeners();
  ...
  _tryHandleResponse(data) {
    if (this._options.extractRequestId) {
      ...
      this._options.isPossibleResponse(data, this._requests.getMetadata(requestId))) {
        this._onResponse.dispatchAsync(data, requestId);
        this._requests.resolve(requestId, data);
      }else{
        this._onRequest.dispatchAsync(data);
      }
    }
  }
